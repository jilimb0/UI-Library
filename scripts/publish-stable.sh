...
run_check() {
  ...
  set +e
  node ./scripts/check-published-code.js | tee "$TMP_CHECK_OUTPUT"
  CHECK_EXIT=$?
  set -e
  node ./scripts/check-published-code.js --json > "$TMP_CHECK_JSON" 2>/dev/null || true
}
...
preview_changelogs() {
  ...
  node ./scripts/bump-package-versions.js --bump="$bump_type" --dry-run < "$TMP_CHECK_JSON"
  echo
